...
# pages_src 폴더를 파이썬의 import 경로(sys.path)에 직접 추가한다.
...
#   - 파일경로는 상대경로. main.py 기준
#   - default=True인 페이지가 앱을 처음 열었을 때 보이는 화면이 된다.
home_page = st.Page('pages_src/home.py', title='홈', icon='🏠', default=True)
explore_page = st.Page('pages_src/explore.py', title='역별탐색', icon='📊')
search.page = st.Page('pages_src/search.py', title='검색(폼)', icon='🔍')
trend.page = st.Page('pages_src/trend.py', title='기간,시간대 추이', icon='📈')
about.page = st.Page('pages_src/about.py', title='소개', icon='ℹ️')
