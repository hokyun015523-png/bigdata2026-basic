...
작업일자" 2026-08-03
작업자: 장호균
작업 목적: train.py로 만든 churn_model.joblib를 실제로 사용하는 사용자화면 코드(웹 페이지 화면), 시각적으로 보여준다.
...
# 브라우저 탭 제목과 아이콘 설정 - 반드시 다른 st.명령어 보다 먼저 호출해야한다.
...
with st.form('customer'):
    # number_input 위젯 : (레이블-> 문구, 최솟값, 최댓값, 기본값)
    ...
    # selectbox 위젯 : 미리 정해진 값 중 하남나 고르게 한다.
    #                  train.py에서 학습할 때 지정한 테이터와 같아야 한다.
...
if submitted: 
    ...
    # predict_proba(row): [[비이탈 확률, 이탈 확률]] 형태의 차원 배열 반환
