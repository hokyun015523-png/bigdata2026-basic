...
def save_csv(data, filename):
    """
    결과로 나온 딕셔너리 리스트를 csv파일로 저장한는 함수들
    ...
    newline='' : csv모듈이 줄바꿈을 직접 처리하므로 빈 문자열로 설정 
                윈도 상태에서 빈줄이 들어갈 가망성이 아주 크다.
    ...
    with open(filename, 'wt', newline='', encoding='utf-8-sig') as f:
        ...
        writer.writeheader( )#헤더 컬럼명 한줄 쓰기
        writer.writerows(data) #데이터 전체 쓰기
    print(f'저장완료: {filename} ({len(data)}행)')
...
#저장파일명
OUTPUT_FOLE = f'{TARGET}_월별승차추이.csv'

#함수 호출
save_csv(result_data, OUTPUT_FILE)

print()
print(f'저장된 파일을 확인해보세요 : {OUTPUT_FOLE}')
