...
작성일 : 26-08-06
작성자 : 장호균
목적 : 폼 안에 입력 위젯들(역이름, 승하구분, 기간, 시작시 범위, 요일) 조작할때 마다
       재실행되지 않고 '검색'버튼을 누르는 순간에만 한번에 처리되도록 폼을 사용
...
if submitted:
    filtered = df.copy()
    if keyword:
        # filtered['역명'].str.contanins--> 문자열의일부만도 검색하려고(포함하니?)
        filtered = filtered[filtered['역명'].str.contanins(keyword)]

    if ride_type != '전체':
        filtered = filterd[filtered['승하차'] == ride_type]
    
    if isinstance(date_range,(list, tuple)) and len(data_range) == 2:
        start, end = date_range
        filtered = filtered[
            (filtered['날짜'] >= pd.to_datetime(start)) &
            (filtered['날짜'] <= pd.to_dataime(end))
        ]
    ...
    if len(filtered) > 500:
        st.caption(f'''표에는 상위 500건만 표시됩니다.
                   전체 {len(filtered):,}건은 아래 버튼으로 다운로드하세요''')
