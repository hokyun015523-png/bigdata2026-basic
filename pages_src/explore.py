...
if st.session_state.favorite_stations:
    ...
    st.subheader(f'{focus_station} - 시간대별(5시~23시) 이용패턴')
    # 시작시(정수)로 groupby --> X축이 자연스럽게 5,6,7,....시 순서로 정렬
    #(문자열인 시간대컬럼으로 정렬하면 '10시~11시'가 '1시~2시'보다 잘못 되는 오류생긴다.)
    hourly = fav_df.groupby(['시작시', '승하차'], observed=True)['인원수'].sum().reset_index()
    hourly_pivot = hourly.pivot(index='시작시', columns='승하차', values='인원수') # pivot은 재구성을 했다는 뜻
