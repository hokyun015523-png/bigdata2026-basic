...
작업일자" 2026-08-03
작업자: 장호균
작업 목적: 통신 고객 이탈 분류 모델을 학습하고 평가, 결과와 모델을 저장한다.
...
from sklearn.ensemble import RandomForestClassifier # 랜덤포레스트 분류 모델을 사용
from sklearn.impute import SimpleImputer # 결측치를 대표값으로 채운다. / 숫자는 중앙값, 범주형은 최빈값으로 설정
from sklearn.metrics import classification_report, confusion_matrix, roc_auc_score # 평가지표들(분류들)
...
# .parent : 파일이 들어있는 폴더
HERE = Path(__file__).resolve().parent # 대문자로 작성한것은 상수로 하겠다는 뜻
...
# print(HERE.parent) # C:\Users\Administrator\bigdata2026
...
# 파이프라인 구성
def build_pipline() -> Pipeline:
    """전처리와 모델을 하나로 묶어 학습, api의 처리를 하겠다."""
    ...
    category_pipe=Pipeline([
        ...
        #   사이킷런의 OneHotEncoder 학습 시점의 규칙을 기억한다. 백터로 처리, 파이프라인 단위로 할 때 편리하다.
        # handle_unknown='ignore' : 처음 보는 값이 들어오면, 새 컬럼을 만드는 대신 설정에 따라 모든 값이 0인 벡터로 처리한다.
        ('onehot', OneHotEncoder(handle_unknown='ignore'))
    ])

    #---- 컬럼별로 다른 전처리를 동시에 적용 ----
    # ColumnTransformer : "이 열은 numberic_pip로, 저 열은 catagory_pipe로 한번에 지정해서 병렬 처리"
...
def train() -> dict:
    """ 재현가능한 분할로 모델을 학습하고, 실무 설명용 지표를 반환한다. """
    
    # CSV 파일을 읽어 데이터프레임으로 저장
    data = pd.read_csv(DATA_PATH)

    # 입력(X), 정답(y) 분리 --> X, y = data[FEATURES], data[TARGET] -> 이것도 가능
    x = data[FEATURES]
    y = data[TARGET]

    # 학습용/평가용 데이터 분활 --> 전체의 25%를 평가용
    X_train, X_test, y_train, y_test = train_test_split(
        x, y, test_size=0.25, random_state=42, stratify=y
    )

    # 파이프 라인 생성 후 학습 데이터로 학습
    pipeline = build_pipline() # 합수 호출
    pipeline.fit(X_train,y_train)

    # predict_proba() : 각 클래스(0,1)에 속할 확률을 반환 (0:이탈 안한다, 1:이탈 한다)
    #                   [:, 1] 모든행, 1번 컬럼
    #                   이탈할 확률을 알고 싳다.
    probabilties = pipeline.predict_proba(X_test)[:,1]

    # 확률이 0.5 이상이면 이탈(1), 미만이면 비이탈(0)으로 최종 분류
    predictions = (probabilties >= 0.5).astype(int) # int형 변환
    ...
    metrics = {
        ...
        # roc_auc : 0.5(랜덤 수준) ~ 1.0(완벽) 사이의 값, 임계값(0.5)에 상관없이 모델의 전반적인 분류 성능을 보여주는 지표
        # round(숫자, 4) : 숫자를 소수 넷째자리 까지 반올림한다는 뜻
        # float()를 붙여서 형변환을 하는 이유: 파이썬의 기본 float이 아니라, numpy.float64 타입이라서
        'roc_auc': round(float(roc_auc_score(y_test, probabilties)), 4),
        # 'recall_churn' : <재현율> 실제 이탈 고객중 몇 %를 잡아냈나? -> 이탈 예측에서는 이 값이 중요 (놓치면 안되는 고객이므로)
